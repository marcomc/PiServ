# Implementation Tracks

## Table of Contents

- [Purpose](#purpose)
- [Index](#index)
- [Template](#template)

## Purpose

Implementation tracks turn accepted decisions and runbooks into execution
checklists. Use them for active work that is not yet fully automated.

## Index

| Track | Status |
| --- | --- |
| [External SSD USB link recovery](external-storage-usb-link-recovery.md) | High-priority plan |
| [pCloud `pcloudcc` podcast storage](pcloudcc-podcast-storage.md) | Workload integrated |
| [Google Drive and external storage](google-drive-and-external-storage.md) | Proposal |
| [Hermes Agent framework](hermes-agent-framework-research.md) | 0.5 acceptance harness passed; policy and restart/restore gates remain |
| [Omar Shahine HomeClaw MCP over SSH](homeclaw-mcp-over-ssh.md) | Proposal: manual Mac Apple Home gateway |
| [Omar Shahine HomeClaw MCP with Supergateway](homeclaw-mcp-supergateway.md) | Proposal: Tailnet Apple Home gateway |
| [Pi Node on PiServ](pi-node-on-piserv.md) | Investigation: native install blocked |
| [Ansible Galaxy Wi-Fi watchdog role assessment](ansible-galaxy-wifi-watchdog-role-assessment.md) | Decision: retain a dedicated role |

## Template

Each implementation track should include:

- Scope and non-goals.
- Known inputs and decisions.
- Ordered phases.
- Validation gates.
- Automation follow-up.
- Rollback notes.
- Evidence log.
